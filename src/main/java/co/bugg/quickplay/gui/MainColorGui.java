...
public class MainColorGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        for(HashMap.Entry<String, QuickPlayColor> entry : QuickPlay.configManager.getConfig().colors.entrySet()) {
            buttonList.add(new GuiButton(buttonId, width / 2 - 100, (int) (height * 0.4) - 10 + (25 * buttonId), new TextComponentTranslation("quickplay.color.name." + entry.getValue().getUnlocalizedName()).getFormattedText()));
