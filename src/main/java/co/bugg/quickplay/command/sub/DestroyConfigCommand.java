...
public class DestroyConfigCommand extends QpSubCommand {
    ...
    public void run(ICommandSender sender, String[] args) {
        if(args.length < 2) {
            sender.sendMessage(new TextComponentTranslation("quickplay.command.destroyconfig.config_reset_confirm", ChatFormatting.RED + Reference.MOD_NAME, ChatFormatting.AQUA + getConfirmCommand()));
        } else {
            if(args[1].equalsIgnoreCase(confirmWord)) {
                QuickPlay.configManager.resetConfig();
                sender.sendMessage(new TextComponentTranslation("quickplay.command.destroyconfig.config_reset"));
            } else {
                sender.sendMessage(new TextComponentTranslation("quickplay.command.destroyconfig.config_reset_confirm", ChatFormatting.RED + Reference.MOD_NAME, ChatFormatting.AQUA + getConfirmCommand()));
            }
        }
